...
from models.rnd import RandomRec
...
def create_model(data, seed, verbose=False):
    model = RandomRec(data, seed=seed)
...
def test_model_factory(dataset, config, args):
    ...
    model = create_model(train_data, args.seed, verbose=args.verbose)
    model.build()
    return model